...
btnClose.addEventListener('click', function (e) {
  ...
  if (
    ...
    //Find index of the current account to use for deleting
...
//Excersize 2 (count how many deposits with at least 1000 dollars)
const numDepositsThousand1 = accounts
  .flatMap(acc => acc.movements)
  .filter(mov => mov >= 1000).length;
console.log(numDepositsThousand1);

//with reduce instead of filter
const numDepositsThousand2 = accounts
  .flatMap(acc => acc.movements)
  .reduce((count, cur) => (cur >= 1000 ? count + 1 : count), 0);
console.log(numDepositsThousand2);

//the plus operator would not work in the above example where we use +1 because:
let a = 10;
//nothing gets added
console.log(a++);
//now it adds a value
...
//But it does work if we use ++ as a prefix operator
let b = 10;
//prefix ++
console.log(++b);

//Excersize 3 with reduce, creating an object
const sums = accounts
  .flatMap(acc => acc.movements)
  .reduce(
    (sums, cur) => {
      // if you have a function body {} you have to explicitly return
...
//Destructuring the object into its own variables
const { deposits, withdrawals } = accounts
  ...
  .reduce(
    (sums, cur) => {
      // if you have a function body {} you have to explicitly return
      //cur > 0 ? (sums.deposits += cur) : (sums.withdrawals += cur);
      //beneath is a less duplicate way to solve the same thing we dont repeat sums
...
//Excersize 4 capitalize some letters not all
...
console.log(
  convertTitleCase(
    'and this is a LONGER title and its longer than the previous one'
  )
);
console.log(convertTitleCase('this is a gOOd title its an amazing one'));
*/
...
//In a function so its usable by other objects
const rightAmountDogFood = function (obj) {
  if (obj.curFood > obj.recFood) {
    console.log(
      `You are feeding your dog ${obj.curFood - obj.recFood} grams too much`
    );
  } else {
    console.log(
      `You are feeding your dog ${obj.recFood - obj.curFood} grams too little`
    );
  }
};

rightAmountDogFood(dogSarah);

//3. owners of dogs that eat too much owners of dogs that eat to little in new arrays
...
//4. String of owners feeding too little or too much
console.log(`${ownersEatTooMuch.join(' and ')}'s dogs eat too much`);
console.log(`${ownersEatTooLittle.join(' and ')}'s dogs eat too much`);

//5. if you read the word 'any' you can think of the some function if some of the values are true
console.log(dogs.some(dog => dog.curFood === dog.recFood));

//6. Ok, amount: current > (recommended * 0.90) && current < (recommended * 1.10)
const checkEatingOk = dog =>
  dog.curFood > dog.recFood * 0.9 && dog.curFood < dog.recFood * 1.1;

//. filter the dogs that eat the right amount
console.log(dogs.some(checkEatingOk));

//7. array with only the dogs that eat the right amount
console.log(dogs.filter(checkEatingOk));

//8. make dogs array copy and sort by right amount
